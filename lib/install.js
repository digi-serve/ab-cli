...
async function cloneRepo() {
   console.log("... cloning runtime repo");
   ...
   if (Options.runtime) {
      console.log(`  ... switching to branch ${Options.runtime}`);
...
async function installDependencies() {
   ...
   return utils.logFormatter.exec(
...
async function compileUI() {
   ...
   const mode = Options.develop ? "dev" : "prod";
   await utils.logFormatter.exec(
      `node node_modules/webpack-cli/bin/cli.js -c webpack.${mode}.js`,
      Options.verbose,
      { stdout: "grey" }
   );
   shell.popd("-q");
...
function configTenantAdmin(done) {
   const tenantOptions = Options.tenant || {};
   tenantOptions.verbose = Options.verbose;
   TenantAdmin.run(tenantOptions)
      .then(() => {
         done();
      })
      .catch(done);
...
function devInstallEndMessage() {
   ...
   console.log(
      `\u2551  ${chalk.bgWhite.cyan(" $")}${chalk.bgWhite.black(
         ` cd ${utils.logFormatter.adjustLength(" ./UP.sh", 27)}`
      )}       \u2551`
   );
   console.log(`\u2551${utils.logFormatter.adjustLength("", 42)}\u2551`);
   console.log(
