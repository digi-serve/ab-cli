 ...
 * @param {obj} options provided parameters for this command
 */
    ...
    if (options[param]) {
      try {
        if (!fs.statSync(options[param]).isFile()) {
          console.log(
            `${param} : ${chalk.yellow.bold(options[param])} is not a file.
            `
          );
    ...
    .prompt([
      {
        ...
        when: values => {
          return (
            !values.process && !options.self && !options.none && !options.exist
          );
        }
      },
      {
      ...
      },
      {
        ...
        when: values => {
          // only display this question if we answered 'self' to 1st prompt
          return values.process == "self" && !options.sslOrganizationName;
        }
      },
      {
        name: "sslUnitName",
      ...
      },
      {
        ...
        validate: value => {
          return value.length >= 4 ? true : "challenge must be >= 4 bytes long";
        }
      },

      {
        ...
        when: values => {
          ...
          return (
            ...
            (!options.pathKey || !fs.statSync(options.pathKey).isFile())
          );
        },
        validate: value => {
      ...
      },
      {
        ...
        when: values => {
          ...
          return (
            values.process == "exist" &&
            (!options.pathCert || !fs.statSync(options.pathCert).isFile())
          );
        }
      }
    ])
 ...
 * @param {fn} done a callback(err) to call.
...
function processSelfSignedCert(done) {
  console.log("processSelfSignedCert()");
  var cwd = process.cwd();
  async.series(
    [
      ...
      next => {
        ...
        utils
          .execCommand({
            ...
            responses: {
              "(2 letter code) []:": Options.sslCountryName + "\n", // Country Name
              "(full name) []:": Options.sslProvinceName + "\n", // State or Province Name
              "city) []:": Options.sslLocalityName + "\n", // Locality Name
              "company) []:": Options.sslOrganizationName + "\n", // Organization Name
              "section) []:": Options.sslUnitName + "\n", // Organizational Unit Name
              "host name) []:": Options.sslCommonName + "\n", // Common Name
      ...
      next => {
        ...
        shell.exec("openssl x509 -in server.crt -out server.pem -outform PEM");
      ...
      next => {
        process.chdir(cwd);
        async.series([patchClearSSLEntries, patchUseDefaultConfig], err => {
          next(err);
        });
      }
    ],
