  ...
  inquirer
    .prompt([
      ...
      },
      {
        ...
        when: values => {
          return (
            Options.canEncrypt && !values.encryption && !Options.encryption
          );
        }
      },
      {
...
function setProperDBconfig(done) {
  var config = `source: ./mysql/conf.d/my-noencrypt.cfg`;
  if (Options.encryption) {
    config = `source: ./mysql/conf.d/my.cfg`;
  }
  ...
  utils.filePatch(
    [
      ...
      {
        file: path.join(process.cwd(), "docker-compose.dev.yml"),
        tag: /source:\s*.\/mysql\/conf\.d\/my-*\w*\.cfg/,
        replace: config,
        log: ""
      },
      // patch our docker-compose.yml
...
3:${key3}`;
  ...
  shelljs.exec(
    `openssl enc -aes-256-cbc -md sha1 -k ${
      Options.encryptionPword
    } -in ${pathRawKeys} -out ${pathEncrKeys}`
  );
  ...
  fs.writeFileSync(
    path.join(process.cwd(), "mysql", "key", ".key"),
    Options.encryptionPword
  );
