 ...
 * @param {cb(err)} done
 */
async function setupFileEncryptionKeys() {
   ...
   var key1 = await utils.logFormatter.exec("openssl rand -hex 32", verbose);
   var key2 = await utils.logFormatter.exec("openssl rand -hex 32", verbose);
   var key3 = await utils.logFormatter.exec("openssl rand -hex 32", verbose);
