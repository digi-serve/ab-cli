   ...
   inquirer
      .prompt([
         ...
         },
         {
            ...
            default: `http://localhost:${Options.port}`,
            when: (values) => {
...
function updateSQLInsert(done) {
   ...
   var patches = [
      {
         file: path.join(process.cwd(), "mysql", "init", "02-site_tables.sql"),
         tag: /##admin-url##/g, // <-- Global
         replace: Options.url,
         log: "patching: 02-site_tables.sql connect URL to Tenant(Admin)",
      },
      {
