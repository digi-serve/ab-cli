    ...
    return new Promise((resolve, reject) => {
        async.series(
            [
                ...
                removeFiles,
                copyTemplates
                //// **** git install our appbuilder_platform_mobile project
            ],
            (err) => {
                if (err) {
                    reject(err);
...
function copyTemplates(done) {
    var copyTheseFiles = ["MaterialIcons-Regular", ".css"];
...
function installNodeModules(done) {
    ...
    var fileCopies = [
        ...
        {
            from: path.join(nodeDir, "framework7", "js", "framework7.min.js"),
            to: path.join(jsDir, "framework7.min.js")
        },
        {
