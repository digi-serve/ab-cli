...
function installGitDependencies(done) {
    shell.pushd(path.join(process.cwd(), "developer", Options.name));
    ...
    if (process.platform == "win32") {
        shell.exec(
            `docker run --mount type=bind,source=%cd%,target=/app -w /app node yarn`
        );
    } else {
        shell.exec(
            `docker run --mount type=bind,source="$(pwd)",target=/app -w /app node yarn`
        );
    }
    ...
    inquirer
        .prompt([
            ...
            },
            {
                ...
                when: (values) => {
                    return (
                        !values.author && typeof Options.author == "undefined"
                    );
                }
            },
            {
                ...
                when: (values) => {
                    return (
                        !values.serviceKey &&
                        typeof Options.serviceKey == "undefined"
                    );
                }
            },
            {
