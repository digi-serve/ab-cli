      ...
      async.series(
         [
            (done) => {
               ...
               const command =
                  platform === "podman"
                     ? `podman compose -f ${composeFileName} -p ${stack} down`
                     : `docker stack rm ${stack}`;
               shelljs.exec(command, { silent });
               done();
            },
            ...
            (done) => {
               ...
               var tryIt = () => {
                  ...
                  shelljs.env["AB_DB_VERSION"] = Options.tag_db ?? Options.tag;
                  const command =
                     platform === "podman"
                        ? `podman compose -f ${composeFileName} -p ${stack} up -d`
