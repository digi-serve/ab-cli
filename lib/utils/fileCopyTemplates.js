...
module.exports = function(templateDir, data, listCopies, cb) {
    ...
    if (fs.existsSync(templatePath)) {
        try {
            recursiveScan(data || {}, templatePath, null, listCopies);
            cb();
        } catch (e) {
            cb(e);
        }
    ...
    for (var f in files) {
        ...
        if (skipFiles.indexOf(fileName) == -1) {
            ...
            var stats = fs.statSync(
                path.resolve(templatePath, templateFilePath)
            );

            // find path to where new instance should be:
            var newFilePath = stringRender(
                path.resolve(currPath, fileName),
                data
            );
                ...
                if (!fs.existsSync(newFilePath)) {
                    ...
                    if (!shouldCopyFile(newFilePath, listCopies)) {
                        ...
                        try {
                            ...
                            console.log("Error: file:" + displayPath);
                            throw e;
                        }
                    } else {
                        // copy file
