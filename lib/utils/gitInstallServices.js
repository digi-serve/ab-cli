...
module.exports = function (allServices = [], options = {}, done) {
   ...
   if (!options.silent) {
      log("... installing git repos");
   } else {
      log("... installing git repos (silent)");
   }
         ...
         async.series(
            [
               (next) => {
                  ...
                  const bar = progressBar("    git clone   ");
                  bar.start(100, 0, { filename: s });
                  ...
                  if (oldURLs.indexOf(s) != -1) {
                     gitURL = `https://github.com/appdevdesigns/${repoName}.git`;
                  }
                  const command = shell.exec(
                     `git clone --recursive --progress ${gitURL} ${s}`,
                     { silent: true },
                     () => {
                        bar.update(100);
                        bar.stop();
      ...
      (err) => {
         ...
         async.series(
            [
               ...
               (next) => {
                  console.log(`... npm install (takes a while)`);
                  const bar = progressBar("    npm install ");
                  bar.start(100, 0);
                  ...
                  execCommand({
                     ...
                     onData: (data) => {
                        ...
                        if (context.total > 0) {
                           bar.update((context.done / context.total) * 100, {
                              filename: context.service ? context.service : "",
                           });
                        }
                     },
                     outputOnStdErr: true,
               ...
               (next) => {
                  console.log(`... remove Install script`);
                  // remove the script
