/**
 * LogFormatter
 * helpers to ensure a consistant look to our output
 */
...
let verbose = false;
/**
 * set the logging mode to verbose
 */
function setVerbose() {
   verbose = true;
}
 ...
 * @returns {string}
 */
function format(text, { color, pad } = {}) {
 ...
 * @param {string="gray"} options.outColor log color for stdout (chalkjs color)
 * @param {string="    "} options.pad text to add to the let of the text
 * @param {boolean} options.hideStderr no stderr unless in verbose mode
 * @param {boolean} options.showAll ignore the verbose mode and always output
 ...
 * @returns {cliProgress.SingleBar} instance of a cliProgress bar
 */
